...
INSERT INTO teaching_activity(activity_name, factor, is_derived) VALUES
 ...
 ('Exercise', 2.00, FALSE) -- REQUIRED FOR TASK 3
ON CONFLICT (activity_name) DO NOTHING;

-- Derived activity coefficients
...
-- Rule parameter
INSERT INTO allocation_rule(rule_id, max_instances_per_period) VALUES (1, 4)
...
-- Auto-generate planned rows for derived activities (Administration, Examination, etc.)
-- so allocations and reports can reference them through the foreign key on planned_activity.
WITH derived_hours AS (
    SELECT ci.instance_id AS course_instance_id,
           ta.activity_id,
           dac.const + dac.hp_coeff * clv.hp + dac.students_coeff * ci.num_students AS planned_hours
    FROM course_instance ci
    JOIN course_layout_version clv
      ON clv.course_code = ci.course_code AND clv.version_no = ci.layout_version_no
    JOIN teaching_activity ta ON ta.is_derived = TRUE
    JOIN derived_activity_coeffs dac ON dac.activity_id = ta.activity_id
)
INSERT INTO planned_activity(course_instance_id, activity_id, planned_hours)
SELECT course_instance_id, activity_id, planned_hours FROM derived_hours
