...
-- Teaching activities
-- NOTE: 'Exercise' added for Task A requirement 4
...
-- Exercise is pre-seeded so script-only verification works
INSERT INTO teaching_activity(activity_name, factor, is_derived) VALUES
 ('Exercise', 2.00, FALSE)
ON CONFLICT (activity_name) DO NOTHING;

-- Associate Exercise with IV1351 P1 (idempotent)
INSERT INTO planned_activity(course_instance_id, activity_id, planned_hours)
SELECT '2025-50001', activity_id, 30 FROM teaching_activity WHERE activity_name='Exercise'
ON CONFLICT (course_instance_id, activity_id) DO NOTHING;

-- Allocate teacher (employee 6) to Exercise (idempotent)
INSERT INTO allocation(employee_id, course_instance_id, activity_id, salary_version_id, allocated_hours)
SELECT 6, '2025-50001', ta.activity_id,
       (SELECT salary_version_id FROM employee_salary_history WHERE employee_id = 6 AND version_no = 1),
       60
FROM teaching_activity ta WHERE ta.activity_name='Exercise'
ON CONFLICT (employee_id, course_instance_id, activity_id) DO NOTHING;
