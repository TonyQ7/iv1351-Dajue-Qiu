 ...
 * Update, Delete).
 * 
 * IMPORTANT: DAO methods do NOT commit or rollback transactions.
 * TransactionManager handles all transaction boundaries (commit/rollback).
 */
public class TeachingDAO {
     ...
     * Caller must first lock the employee row via readEmployeeForUpdate() to
    ...
    public void updateCourseInstanceStudents(CourseInstanceDTO instance) throws TeachingDBException {
        ...
        try {
            ...
            // TransactionManager handles commit
        } catch (SQLException sqle) {
            handleException(failureMsg, sqle);
        }
    }
    ...
    private void handleException(String failureMsg, Exception cause) throws TeachingDBException {
        // Note: TransactionManager handles rollback - DAO only throws
        if (cause != null) {
